...
The Salesforce Brilliant Mule Customer System API provides access to customer data stored in the Salesforce Brilliant Mule instance. This API simplifies integration with Salesforce by maintaining consistency with established customer account concepts and structures. Customer data is presented through a clear, system-aligned data model designed for consumption by upstream Process APIs within the Customer API-led reference architecture.

## Purpose

This System API abstracts the complexity involved in integrating with the Salesforce Brilliant Mule instance and its data model, providing a reusable, system-aligned interface. By encapsulating system-specific complexity behind a stable interface, the API simplifies integration with the Salesforce Brilliant Mule instance while maintaining fidelity to the source system.

## Capabilities

The Salesforce Brilliant Mule Customer System API currently offers the following functionalities:
...
The System API functions primarily as a request-response interface for retrieving and synchronizing customer data with the Salesforce Brilliant Mule instance over HTTPS. In the current implementation, the Mule application also listens for and consumes Salesforce Change Data Capture (CDC) events related to Account changes in near real time. While this event consumption facilitates synchronization and internal processing scenarios, it remains an implementation detail rather than a client-facing feature of the API. Consequently, this specification models only the HTTP resources and payloads available to consumers, with CDC behavior included solely for architectural context.

## Data Model

Customer data exchanged through this API follows a system-aligned customer model derived from the Salesforce Brilliant Mule instance customer account concepts and structures. This System API is intentionally system-aligned rather than canonical. Canonical transformation and cross-system normalization belong in the Process API layer.
...
The Salesforce Brilliant Mule Customer System API, within the System API layer, enables access to the Salesforce Brilliant Mule instance customer data in the Customer API-led reference architecture. The Customer Process API handles orchestration and canonical transformation across systems. The diagram below shows the Salesforce Brilliant Mule Customer System API’s role in the architecture.

![Salesforce Brilliant Mule Customer System API in Customer API-Led Reference Architecture](resources/sf-brilm-customer-sapi-diagram-6919f597-42a6-424e-ba41-3a259791569b.png)

In this architecture, the Salesforce Brilliant Mule Customer System API remains system-aligned, preserving the Salesforce Brilliant Mule instance customer account structures while enabling the Process API to normalize and coordinate data across Salesforce, SAP ECC, and other systems.
